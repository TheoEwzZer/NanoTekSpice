...
nts::Gate4514::Gate4514() : mPreviousStrobe(TRUE)
{
    update();
}
...
void nts::Gate4514::update()
{
    ...
    if ((mPins[23] == FALSE) && (mPins[1] == FALSE)) {
...
nts::Tristate nts::Gate4514::compute(const size_t pin)
{
    ...
    if ((mPins[1] == TRUE) || (mPins[1] == UNDEFINED)
    || (mPins[23] == UNDEFINED)) {
        mPreviousStrobe = mPins[23];
        return UNDEFINED;
    }
    if ((mPins[23] == TRUE) && (mPreviousStrobe == FALSE)) {
        mPreviousStrobe = mPins[23];
        update();
        return mPins[pin];
    }
