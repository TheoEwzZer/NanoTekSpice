...
void nts::Parsing::processChipsetsSection(const string &line, smatch &match)
{
    ...
    if (mComponentNames.find(name) != mComponentNames.end()) {
...
void nts::Parsing::linkComponents(nts::IComponent &component1, const size_t pin1,
...
void nts::Parsing::processLinksSection(const string &line, smatch &match)
{
    ...
    if (mComponentNames.find(componentName1) == mComponentNames.end()) {
        throw nts::Error("Unknown component name '" + componentName1 + "'");
    }
    if (mComponentNames.find(componentName2) == mComponentNames.end()) {
