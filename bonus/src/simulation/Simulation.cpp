...
void nts::Simulation::change(const string &input, const string &value)
{
    ...
    if (components.find(input) != components.end()) {
        if (value == "0") {
...
{
    nts::Tristate inputVal = nts::Tristate::UNDEFINED;
    nts::Tristate outputVal = nts::Tristate::UNDEFINED;
    ...
    for (const auto &inputName: inputNames) {
        ...
        cout << "  " << inputName << ": ";
        if (inputVal == nts::Tristate::UNDEFINED) {
            cout << "U";
        } else {
    ...
    for (const auto &outputName: outputNames) {
        ...
        cout << "  " << outputName << ": ";
        if (outputVal == nts::Tristate::UNDEFINED) {
            cout << "U";
        } else {
